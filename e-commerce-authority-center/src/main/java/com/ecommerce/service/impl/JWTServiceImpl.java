...
public class JWTServiceImpl implements IJWTService {
    ...
    public String registerUserAndGenerateToken(UsernameAndPassword usernameAndPassword) throws Exception {
       // 先去校验用户是否存在，如果存在，不能重复注册
        ...
        ecommerceUser.setPassword(usernameAndPassword.getPassword()); // MD5 编码后的密码
        ecommerceUser.setExtraInfo("{}");

        // 注册一个新用户，写一条记录到数据表中
        // 通过dao方法，返回的ecommerceuser是带id的ecommerce user
